...
import { Search, ArrowRight, Package, ShoppingCart, Factory, User, Hash, Command, Zap, CornerDownLeft, FlaskRound, MapPin, ShieldCheck } from 'lucide-react';
...
const CommandPalette: React.FC<CommandPaletteProps> = ({ isOpen, onClose, onNavigate, inventory, orders, jobs }) => {
  ...
  const sections = [
    {
      title: 'Quick Actions',
      items: [
        { id: 'nav-dash', label: 'Go to Dashboard', icon: <Command className="w-4 h-4" />, action: () => onNavigate('DASHBOARD'), shortcut: 'G D' },
        { id: 'nav-inv', label: 'Go to Inventory', icon: <Package className="w-4 h-4" />, action: () => onNavigate('INVENTORY'), shortcut: 'G I' },
        { id: 'nav-prod', label: 'Go to Production', icon: <Factory className="w-4 h-4" />, action: () => onNavigate('PRODUCTION'), shortcut: 'G P' },
        { id: 'nav-samp', label: 'Go to Sampling', icon: <FlaskRound className="w-4 h-4" />, action: () => onNavigate('SAMPLING'), shortcut: 'G S' },
        { id: 'nav-track', label: 'Go to Tracking', icon: <MapPin className="w-4 h-4" />, action: () => onNavigate('TRACK_LOTS'), shortcut: 'G T' },
        { id: 'nav-qual', label: 'Go to Quality', icon: <ShieldCheck className="w-4 h-4" />, action: () => onNavigate('QUALITY'), shortcut: 'G Q' },
        { id: 'nav-ord', label: 'Go to Orders', icon: <ShoppingCart className="w-4 h-4" />, action: () => onNavigate('ORDERS'), shortcut: 'G O' },
      ]
    },
    {
      title: 'Recent Orders',
