...
const JobWorkComp: React.FC<JobWorkProps> = ({ 
  jobs, designs = [], inventory = [], onAdd, onUpdate, onDelete, currency = '₹' 
}) => {
  ...
  return (
    <div className="space-y-6 h-full flex flex-col bg-[#f0f2f5] dark:bg-slate-950 -m-8 p-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 shrink-0">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 dark:text-white uppercase tracking-tight">JobWork Matrix</h2>
          <p className="text-xs text-slate-500 mt-1 uppercase tracking-wider font-bold">Consumption Reconciliation & Fabric Yield Analytics</p>
        </div>
        <div className="flex gap-3">
          ...
          >
             <Plus className="w-4 h-4" /> New Challan Node
          </button>
        </div>
      </div>
                  ...
                  <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                          {filteredJobs.map(job => {
                             ...
                             const design = designs.find(d => d.name === firstItemDescription || d.sku === firstItemDescription);
                            ...
                            >
                                ...
                                {design?.imageUrl && (
                                  <div className="w-full aspect-video rounded-xl overflow-hidden border border-slate-100 dark:border-slate-800 mb-4 bg-slate-50 dark:bg-slate-950">
                                     <img src={design.imageUrl} alt="" className="w-full h-full object-cover transition-transform group-hover:scale-105" referrerPolicy="no-referrer" />
                                  </div>
                                )}
